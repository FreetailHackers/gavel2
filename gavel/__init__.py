...
eventlet.monkey_patch(
    os=True, select=True, socket=True, thread=True, time=True, psycopg=True
)
...
bundles = {
    "scss_all": Bundle(
        "generated.scss", depends="**/*.scss", filters=("libsass",), output="all.css"
    ),
    "admin_js": Bundle(
...
def inject_context():
    return dict(
        ...
        finished_button_text=str(
            "Finish Review" if settings.VIRTUAL_EVENT else "Done With Visit"
        ),
...
socketio = SocketIO(
    app, async_mode=async_mode, message_queue=SOCKETIO_REDIS_URL, async_handlers=True
)
