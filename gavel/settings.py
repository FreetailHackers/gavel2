...
def _bool(truth_value):
    ...
    if isinstance(truth_value, str):
        ...
        return lower.startswith("t") or lower.startswith(
            "y"
        )  # accepts things like 'yes', 'True', ...
...
BROKER_URI = c.get(
    "broker_uri", ["REDIS_URL", "BROKER_URI"], default="redis://localhost:6379/0"
)
...
LOGGED_OUT_MESSAGE = c.get(
    "logged_out_message", default=constants.DEFAULT_LOGGED_OUT_MESSAGE
)
