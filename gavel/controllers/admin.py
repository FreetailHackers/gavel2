...
from flask import redirect, render_template, request, url_for, json, jsonify, Response
...
def admin_items():
    ...
    response = app.response_class(
        response=json.dumps(dump_data), status=200, mimetype="application/json"
    )
...
def admin_flags():
    ...
    dump_data = {"flags": flags_dumped, "flag_count": flag_count}
...
def admin_annotators():
    ...
    dump_data = {"annotators": annotators_dumped, "anotator_count": annotator_count}
...
def annotator_patch():
    def tx():
        ...
        if not item:
            return utils.user_error(
                "Annotator %s not found " % request.form["annotator_id"]
            )
        if "name" in request.form:
    ...
    with_retries(tx)
    return redirect(
        url_for("annotator_detail", annotator_id=request.form["annotator_id"])
    )
...
def item_detail(item_id):
    ...
    if not item:
        ...
        return render_template(
            "admin_item.html", item=item, assigned=assigned, skipped=skipped
        )
