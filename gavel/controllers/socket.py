    ...
    try:
        ...
        if str(name) == "annotator":
            return {
                "type": name,
                "target": json.dumps(
                    injectAnnotator(target, annotator_schema.dump(target))
                ),
            }
        elif str(name) == "flag":
            return {
            ...
            return {"type": name, "target": json.dumps(target.to_dict())}
    except Exception as e:
        print(str(e))
        return {"type": "ERROR", "target": json.dumps({"error": "true"})}


def injectAnnotator(target, target_dumped):
    count = Decision.query.filter(Decision.annotator_id == target.id).count()
    target_dumped.update({"votes": count})
...
def injectItem(target, target_dumped):
    ...
    target_dumped.update(
        {
            "viewed": viewed,
            "votes": Decision.query.filter(
                or_(Decision.winner_id == target.id, Decision.loser_id == target.id)
            )
            .distinct(Decision.id)
            .count(),
            "skipped": skipped,
        }
    )
    ...
    try:
        ...
        for i in items:
            socketio.emit(
                ...
                {
                    "type": "item",
                    "target": json.dumps(injectItem.delay(i, item_schema.dump(i))),
                },
                namespace="/admin",
            )
    except Exception as e:
...
def item_listen_delete(mapper, connection, target):
    print(str(target), str(mapper))
    socketio.emit(
        ITEM_DELETED, {"target": item_schema.dump(target)}, namespace="/admin"
    )
